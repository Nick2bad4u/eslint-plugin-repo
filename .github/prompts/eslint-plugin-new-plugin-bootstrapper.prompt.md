...
description: "🤖🤖 Use this prompt to scaffold a brand-new ESLint plugin repository from my modern template. IMPORTANT: treat eslint-plugin-repo-compliance only as a structural and quality guide, never as rule content to keep, rename, or adapt unless I explicitly ask for a similar rule."
...
The current repository may have been scaffolded from my `eslint-plugin-repo-compliance` template, but that does **not** mean you should keep, port, rename, reinterpret, or adapt the TypeFest-specific rules into the new plugin unless I explicitly ask for a similar rule.
...
- do not keep `eslint-plugin-repo-compliance` rules as starter rules for the new plugin
...
1. The root of this repository has been scaffolded using my template (based on my `eslint-plugin-repo-compliance` repo). All root `package.json` dependencies & devDependencies are already installed unless the user says otherwise.
...
6. **Establish the new plugin identity:**
   ...
   - Remove or update all stale references to `eslint-plugin-repo-compliance`, `typefest`, and `ts-extras`.
...
A. The repository is fully re-identified as the new plugin rather than `eslint-plugin-repo-compliance`.
...
F. The entire project matches the layout, doc standards, and coding standards of my `eslint-plugin-repo-compliance` template **without importing unrelated TypeFest-specific rule content**.
